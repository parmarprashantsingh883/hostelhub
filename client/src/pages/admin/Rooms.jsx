  ...
  return (
    <div className="space-y-7">
      <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500">
        <span className="inline-flex items-center gap-1.5"><span className="h-3 w-3 rounded-md bg-night-800" /> Occupied</span>
                  ...
                  <div key={r._id} className="rounded-2xl border border-slate-200/70 dark:border-white/10 bg-white dark:bg-surface p-4 shadow-card">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      {Array.from({ length: r.capacity }).map((_, i) => {
                        ...
                        if (t) return <span key={i} title={t.name} className="flex h-11 w-11 items-center justify-center rounded-xl bg-night-800 font-mono text-[11px] font-semibold text-brand-200" aria-label={`Bed occupied by ${t.name}`}>{initials(t.name)}</span>;
                        return (
